...
\textbf{GPT-5.6 Luna} is the outlier in the opposite direction. It executes both domains at 100\% and shows essentially no source discrimination in any metric, except a weak suspicion signal (6/60 vs.\ 0/60, $p=0.027$) that never translates into inspection or refusal. This absence of source-level bias is not calibration; it is blanket carelessness. Inspection and refusal sit at 0\% on \emph{both} domains, so the model was never applying scrutiny to withhold in the first place. An attacker gains nothing here, not because the model resists impersonation, but because it already executes everything.
